...
{
	if (m_buttons.size() != POWER_COUNT)
		return;

    for (uint32_t i = 0; i < sizeof(g_store_to_power)/sizeof(int)/2; i++)
	{
        int ii = g_store_to_power[i][0];
        int si = g_store_to_power[i][1];

		if (!manager.IsLocked(si))
			m_buttons[ii].Unlocked = true;
	}
