...
bool Quadris::CheckForAI()
{
    ...
    {
        m_leftLines--;
        m_ai.RequestSpecificFigure(QuadKind::Line, true);
    }
...
bool Quadris::TestRender()
{
	...
	if (!m_stopPlay)
	{
		if (m_curLevel > 0.0f)
			...
			*m_levelDescreaseRatio*m_storeManager.GetSlowerRatio();
		else
		{
			if (m_currentLevel > 1)
			{
...
void Quadris::LinesFilled(uint32_t n)
{
    ...
    {
        std::stringstream str;
        str << n*((m_currentLevel + 5) * (money - 3)) + m_lineStreak * 5;
        FloatingText::AddFloating(FloatingText::SUPERCASH, str.str(),
            Engine::Instance().GetTouchPoint().X
            , Engine::Instance().GetTouchPoint().Y);

        m_money = m_money + n * ((m_currentLevel+5) * (money - 3)) + m_lineStreak*5;
    ...
    m_curLevel += m_levelRatio*(double)n*n*m_lineStreak*m_storeManager.GetAccelRatio();
    if (m_curLevel >= 1.0)
    {
        m_curLevel -= 1.0;
        if (m_curLevel >= 1.0)
...
void Quadris::GetNewFigure()
{
	...
	if (m_leftLines > 0)
	{
		m_ai.RequestSpecificFigure(Line);
		m_leftLines--;
	}
    else if (m_savedFigure)
...
bool Quadris::StoreState()
{
	...
	if (!m_storeManager.IsLocked(m_storeIdx))
	{
